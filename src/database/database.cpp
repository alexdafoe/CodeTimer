...
/*void DataBase::upgradeRow() {
    QString end = "15:34:23";
    int workSec = 8037;
    QString workTime;
    TimeSecToString()(workSec, workTime);
    db.transaction();
    bool sqlState = true;
    QSqlQuery query(db);
    ...
    query.bindValue(":workTime", workTime);
    query.bindValue(":workSec", workSec);
    ...
    qDebug() << "succsess transaction upgrade row 42";
...
/*void DataBase::updateDatabase() {
    ...
    QSqlQuery query(db);
    while(sqlState){
        sqlState = query.exec("CREATE TEMPORARY TABLE tmp ("
                              "id INTEGER PRIMARY KEY, "
                              TABLE_DATE " DATE NOT NULL, "
                              TABLE_STARTTIME " VARCHAR2(20), "
                              TABLE_ENDTIME " VARCHAR2(20), "
                              TABLE_WORKHOURS " VARCHAR2(20) NOT NULL, "
                              TABLE_WORKSECONDS " INTEGER NOT NULL, "
                              TABLE_COMMENTS " VARCHAR2(30) "
                                             ");");
        if(!sqlState){
            qDebug() << "error create tmp table" << query.lastError().text();
            break;
        }
        sqlState = query.exec("INSERT INTO tmp "
                              " SELECT id, " TABLE_DATE ", "
                              TABLE_STARTTIME ", " TABLE_ENDTIME ", "
                              TABLE_WORKHOURS ", " TABLE_WORKSECONDS ", "
                              TABLE_COMMENTS
                              " FROM " TABLE ";");
        if(!sqlState){
            qDebug() << "error insert into tmp from table " TABLE << query.lastError().text();
            break;
        }
        sqlState = query.exec("DROP TABLE " TABLE ";");
        if(!sqlState){
            qDebug() << "error drop table " TABLE << query.lastError().text();
            break;
        }
        sqlState = query.exec("CREATE TABLE " TABLE_NAME " ("
                               "id INTEGER PRIMARY KEY, "
                              _DATE " DATE NOT NULL, "
                              ...
                              _COMMENTS " VARCHAR2(30)"
                               ");" );
        if(!sqlState){
            qDebug() << "error create new table " TABLE_NAME << query.lastError().text();
            break;
        }
        sqlState = query.exec("INSERT INTO " TABLE_NAME " "
                              " (id, " _DATE ", "
                              ...
                              TABLE_DATE ", "
                              TABLE_STARTTIME ", " TABLE_ENDTIME ", "
                              TABLE_WORKHOURS ", " TABLE_WORKHOURS ", "
                              TABLE_WORKSECONDS ", " TABLE_WORKSECONDS ", "
                              TABLE_COMMENTS
                              " FROM tmp;"
                              );
        ...
        sqlState = query.exec("SELECT * FROM tmp;");
        if(!sqlState){
            qDebug() << "error select tmp"<< query.lastError().text();
            break;
        }
        QVector<int> id;
        QVector<int> sec;
        QList<QDate> date;
        QStringList start, end, coding, comm;
        while(query.next()){
            id.append(query.value("id").toInt());
            date.append(query.value(TABLE_DATE).toDate());
            start.append(query.value(TABLE_STARTTIME).toString());
            end.append(query.value(TABLE_ENDTIME).toString());
            coding.append(query.value(TABLE_WORKHOURS).toString());
            sec.append(query.value(TABLE_WORKSECONDS).toInt());
            comm.append(query.value(TABLE_COMMENTS).toString());
        }
        for(int i = 0; i < id.size(); ++i){
            query.prepare("INSERT INTO " TABLE " ("
                          "id, " TABLE_DATE ", "
                          TABLE_STARTTIME ", " TABLE_ENDTIME ", "
                          TABLE_WRITING_CODE ", " TABLE_WORKSECONDS ", "
                          TABLE_COMMENTS ") "
                          "VALUES (:ID, :date, :start, :end, :coding, :sec, :comm);"
                          );
            query.bindValue(":ID", id.at(i));
            query.bindValue(":date", date.at(i));
            query.bindValue(":start", start.at(i));
            query.bindValue(":end", end.at(i));
            query.bindValue(":coding", coding.at(i));
            query.bindValue(":sec", sec.at(i));
            query.bindValue(":comm", comm.at(i));
            sqlState = query.exec();
            if(!sqlState)
                break;
        }
        ...
        if(!sqlState){
            qDebug() << "error insert into new table " TABLE << query.lastError().text();
            break;
        }
        sqlState = query.exec("DROP TABLE tmp;");
        if(!sqlState){
            qDebug() << "error drop tmp" << query.lastError().text();
            break;
        }
        qDebug() << "succsess swap!!";
        break;
    }
    query.clear();
    qDebug() << "try commit";
